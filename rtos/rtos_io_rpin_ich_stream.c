...
ISR(PCINT1_vect) {
	...
	uint8_t ucDataBuffer[]={0x00,0x02,0x00,0x08,0x00,0x08,0x00,0x02,0x00};
	size_t  xBytesSent;
	
	PORTB ^= (1<<PORTB5);
	xBytesSent = xStreamBufferSendFromISR(xStream, ucDataBuffer, DATA_SZ, &xHigherPriorityTaskWoken );
	portYIELD_FROM_ISR();
}
